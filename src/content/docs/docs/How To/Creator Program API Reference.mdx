...
| Channel   | PWM channel to use if type is set to PWM. There are 12 channels available for use. | 1             |
...
Operates a timer. A maximum of 12 timers can be used. The actions are:
...
Flags are integer variables that can be modified and read. There are 12 flags. The options are:
