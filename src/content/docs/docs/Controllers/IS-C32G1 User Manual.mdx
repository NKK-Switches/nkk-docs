---
title: IS-C32G1
description: IS-C32G1 manual.
---
import { LinkCard, Aside } from '@astrojs/starlight/components';

<div style={{ fontSize: '2.5rem', color: 'white'}}> IS-C32G1 Intelligent Controller User Manual </div>

<img src="/src/assets/C3201-top.png" width="400"/>
...
- [General Controller Features](#general-controller-features)
- [Electrical Specifications](#electrical-specifications)
- [Compatible Logic Boards](#compatible-logic-boards)
  - [Logic Boards](#logic-boards)
  - [Signal Booster](#signal-booster)
  - [Connectors](#connectors)
  - [Ribbon Cables](#ribbon-cables)
- [Operational Detail](#operational-detail)
  - [Power-up Sequence](#power-up-sequence)
  - [Main Operational Mode](#main-operational-mode)
  - [LED Backlighting](#led-backlighting)
  - [Attributes](#attributes)
- [Communication Protocol](#communication-protocol)
  - [Image Format](#image-format)
  - [Images, Attributes, and Backlight Numbering](#images-attributes-and-backlight-numbering)
  - [Switch Numbering](#switch-numbering)
  - [Backlights](#backlights)
  - [Flash Memory](#flash-memory)
- [Command List](#command-list)
- [Saving Images/Attributes using Engineering Kits Communicator](#saving-imagesattributes-using-engineering-kits-communicator)
- [Updating with New Firmware](#updating-with-new-firmware)
- [Hardware](#hardware)
  - [Controller Dimensions](#controller-dimensions)
  - [Logic Board Dimensions](#logic-board-dimensions)
...
The IS-C32G1 controls up to 32 LCD 64x32 switches/displays. The IS-C32G1 is designed to be used in many different applications. There is total flexibility with user-defined features that allow the controllers to be programmed for specific applications. The user defined data and set-up are stored in a non-volatile memory and specify the way the system behaves. The firmware can be customized based on customer requirements. Below are current features:

**General features:**
- Control up to 32 LCD 64x32 switches/displays. Two banks of 16.
...
| ---- | ---------------------------- | ----------------- | -------------------------------------------------------------------- |
| 1    | IS-L02A1-CS                  | IS-L02A1-C        | Logic Board, 1x2, Long Travel LCD 64x32, 2SW. Side by side stackable |
| 2    | IS-L02G1-CS                  | IS-L02G1-C        | Logic Board, 1x2, LCD 64x32, 2SW. Side by side stackable             |
| 3    | IS-L02H2-CS                  | IS-L02H2-C        | Logic Board, 1x2, Compact LCD 64x32, 2SW. Side by side stackable     |
| 4    | IS-L04G1-CS                  | IS-L04G1-C        | Logic Board, 2x2, LCD 64x32, 4SW. Side by side stackable             |
| 5    | IS-L16G2-CS                  | IS-L16G2-C        | Logic Board, 4x4, LCD 64x32, 16SW. Side by side stackable            |

All the logic boards for IS15EBFP4RGB-09YN can be populated with the compact switch IS15ESBFP4RGB.

<div style={{ display: "flex", gap: "20px", alignItems: "flex-end" }}>
  <div style={{ textAlign: "center" }}>
    <div style={{ marginBottom: "8px", fontWeight: "bold" }}>IS-L02A1-C</div>
    <img src="/src/assets/IS-L02A1-C.png" width="200"/>
  </div>
  <div style={{ textAlign: "center" }}>
    <div style={{ marginBottom: "8px", fontWeight: "bold" }}>IS-L02A1-CS</div>
    <img src="/src/assets/IS-L02A1-CS.png" width="200"/>
  </div>
  <div style={{ textAlign: "center" }}>
    <div style={{ marginBottom: "8px", fontWeight: "bold" }}>IS-L02H2-CS</div>
    <img src="/src/assets/IS-L02H2-CS.png" width="200"/>
  </div>  
</div>
...
|-----|----------|--------------------------------------------------|
| 1   | Din      | Connected to Din of the first shift register     |
| 2   | GND      | Ground                                           |
| 3   | CLK      | Clock for all shift registers                    |
| 4   | GND      | Ground                                           |
| 5   | NC       | Not connected                                    |
| 6   | Vsup     | 7V to 12V                                        |
| 7   | LP       | Latch all shift register outputs to all drivers  |
| 8   | NC       | Not connected                                    |
| 9   | SCK      | Clock of all SmartDisplays                       |
| 10  | NC       | Not connected                                    |
| 11  | SDI      | Data of all SmartDisplays                        |
| 12  | Vsup     | 7V to 12V                                        |
| 13  | NC       | Not connected                                    |
| 14  | SWRD     | Switch Read bus for all SmartDisplays            |
...
| Pin | Function | Description                                 |
|-----|----------|---------------------------------------------|
| 1   | Dout     | Connected to Dout of the last shift register |
| 2   | GND      | Ground                                      |
| 3   | CLK      | Connected to CLK of J1                      |
| 4   | GND      | Ground                                      |
| 5   | GND      | Ground                                      |
| 6   | Vsup     | 7V to 12V                                   |
| 7   | LP       | Connected to LP of J1                       |
| 8   | NC       | Not connected                               |
| 9   | SCK      | Connected to SCK of J1                      |
| 10  | GND      | Ground                                      |
| 11  | SDI      | Connected to SDI of J1                      |
| 12  | Vsup     | 7V to 12V                                   |
| 13  | GND      | Ground                                      |
| 14  | SWRD     | Connected to SWRD of J1                     |
...
An LED code is one byte. A low bit denotes OFF and a high bit denotes ON. Bits 0 and 1 must be high. There are 64 different colors available.

|       |     |     |     |     |     |     |     |     |
| ----- | --- | --- | --- | --- | --- | --- | --- | --- |
| Bit   | 7   | 6   | 5   | 4   | 3   | 2   | 1   | 0   |
| Color | R   | R   | G   | G   | B   | B   | 1   | 1   |

(B = Blue, G = Green, R = Red)

The picture below shows the available backlight colors for LCD 64x32. The top 2 digits on each color are the codes for that backlight color.

*Backlight Color Code Table*

<img src="/src/assets/color-table-64x32.png" width="1000"/>

The backlight brightness can be adjusted using the “change brightness” command. See the associated [command list](#command-list) for details.
...
<img src="/src/assets/ascii-hex.png" width="700"/>
...
Refer to the IS-C32G1 Command List xlsx file for the full list of commands and examples. The command list as well as example images and attributes can be downloaded here: [IS-C32G1 Documents](https://www.nkkswitches.com/wp-content/uploads/2024/01/IS-C32G1-Documents-Rev-C.zip)
...
1.	Connect IS-C32G1 to a PC via USB. 
...
Make sure the IS-C32G1 is not connected to Engineering Kits Communicator via COM port when using the NKK Firmware Update Utility and vice versa. The COM port can only be connected to one program at a time. 

## Hardware

### Controller Dimensions
Board size: 3.6” x 3.0” (91.44 x 76.2 mm)   
Mounting hole size: 0.125” (3.175 mm) 

<img src="/src/assets/c3201-dimensions.png" width="700"/>
...
Logic board dimensions for L02A1 and L02G1:

<img src="/src/assets/l02g1-dimensions.png" width="600"/>

Logic board dimensions for L02H2:

<img src="/src/assets/l02h2-dimensions.png" width="600"/>

Logic board dimensions for L04G2:

<img src="/src/assets/l04g2-dimensions.png" width="600"/>

Logic board dimensions for L16G2:

<img src="/src/assets/l16g2-dimensions.png" width="600"/>

Logic board dimensions for LBUF01:

<img src="/src/assets/lbuf01-dimensions.png" width="600"/>

## Schematics

IS-C32G1 and IS-C3201 use the same PCB.

<img src="/src/assets/c3201-schematics.png"/>
